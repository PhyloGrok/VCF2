...
esearch -db sra -query "($txid[Organism]) AND (\"biomol dna\"[Filter] AND \"library layout paired\"[Filter] AND \"platform illumina\"[Filter])" | efetch -format docsum | xtract -pattern Runs -ACC @acc  -element "&ACC" > SraList.txt
